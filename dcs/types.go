...
func (c *Cluster) GetMemberAddrWithName(memberName string) string {
	clusterDomain := viper.GetString(constant.KubernetesClusterDomainEnv)
	return fmt.Sprintf("%s.%s-headless.%s.svc.%s", memberName, c.ClusterCompName, c.Namespace, clusterDomain)
}
