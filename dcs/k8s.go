...
import (
	...
	k8sutil "github.com/apecloud/mongodb_plugin/util/kubernetes"
)
...
func NewKubernetesStore() (*KubernetesStore, error) {
	...
	client, err := k8sutil.GetRESTClient()
	if err != nil {
...
func (store *KubernetesStore) DeleteHaConfigMap() error {
	HaConfigName := store.getHAConfigName()
	err := store.clientset.CoreV1().ConfigMaps(store.namespace).Delete(store.ctx, HaConfigName, metav1.DeleteOptions{})
	if err != nil {
		store.logger.Error(err, "Delete HA configmap failed")
	}
	return err
}
...
func (store *KubernetesStore) CreateHaConfig(ClusterInitializeOwner string) error {
	haName := store.getHAConfigName()
	haConfig, _ := store.GetHaConfig()
	if haConfig.resource != nil {
		haConfigMap, configMapAssertionOK := haConfig.resource.(*corev1.ConfigMap)
		appCluster, clusterMapAssertionOK := store.cluster.Resource.(*appsv1alpha1.Cluster)
		if configMapAssertionOK && clusterMapAssertionOK && haConfigMap.CreationTimestamp.Before(&appCluster.CreationTimestamp) {
			store.logger.Info("A previous HA configmap resource exists, delete it", "name", haConfigMap.Name)
			_ = store.DeleteHaConfigMap()
		} else {
			return errors.New("Ha Configmap has been created")
		}
	}
	...
	haConfigMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				"ClusterInitializeOwner": ClusterInitializeOwner,
			},
		},
	}
...
func (store *KubernetesStore) GetHaConfig() (*HaConfig, error) {
	...
	return &HaConfig{
		index:                  configmap.ResourceVersion,
		ClusterInitializeOwner: annotations["ClusterInitializeOwner"],
		ttl:                    ttl,
		enable:                 enable,
...
func getSyncerPort(pod *corev1.Pod) string {
	for _, container := range pod.Spec.Containers {
		for _, port := range container.Ports {
			if port.Name == constant.SyncerHTTPPortName {
